...
$sql = "UPDATE service SET serviceName='$name', bankID='$bankID', description='$desc' WHERE serviceID='$serviceID'";

//Getting auto inserted ID of the service
if ($conn->query($sql) === TRUE) {

  $sql = "DELETE FROM service_data_type WHERE serviceID = $serviceID";
  $conn->query($sql);

  $sql = "DELETE FROM service_hardcopy WHERE serviceID = $serviceID";
  $conn->query($sql);
  ...
  for ($i=0; $i < $numOfCheckBoxes; $i++) {
    ...
    if ($isRequired) {
      ${"sqlBridge$i"} = "INSERT INTO service_data_type VALUES ('$serviceID', '$common_fields[$i]', '1')";
    } else {
      ${"sqlBridge$i"} = "INSERT INTO service_data_type VALUES ('$serviceID', '$common_fields[$i]', '0')";
    }
  ...
  for ($i=0; $i < $numOfFields; $i++) {
    if ($fieldName[$i] != NULL) {
      ...
      ${"sqlBridge$i"} = "INSERT INTO service_data_type VALUES ('$serviceID', '${"last_datatype_id$i"}', 0)";
      $conn -> query(${"sqlBridge$i"});
    }
  }
